...
// Registering o_active here puts it out-of-sync with o_valid (since the latter is not registered). Removing this register
// is likely to have a negative effect on QoR, so to keep these signals in sync at the finish detector, we add a register
// to the valid_out input in acl_task_copy_finish_detector. These two paths (o_active and o_valid) must be kept in sync for
